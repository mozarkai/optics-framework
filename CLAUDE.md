...
3. `BaseRunner.__init__` (`execute.py:488`) runs discovery + loading synchronously:
   ...
   - `_setup_session` → `SessionManager.create_session` (`common/session_manager.py:151`) which builds a `Session` (`session_manager.py:99`): instantiates `EventSDK`, `OpticsBuilder` (`common/optics_builder.py:31`), and via `_get_enabled_config_list` (`session_manager.py:29`) keeps only `DependencyConfig.enabled == True` entries. `Session.__init__` calls `add_driver` / `add_element_source` / `add_text_detection` / `add_image_detection` then `self.optics.get_driver()` to fail fast. The fail-fast is gated by `require_driver` (default `True`): a device-less session (`require_driver=False`, used by the REST dry-run endpoints) skips the "no enabled drivers" `E0501` and `OpticsBuilder.get_driver`/`get_element_source` hand back an **empty `InstanceFallback([])`** (not `None`) when their config is empty, so the keyword registry builds with empty strategy lists and never touches a device.
...
`dry_run` follows the same path with `DryRunExecutor` (`execution.py:92`) and `TestRunner._process_test_case(..., dry_run=True)` — it resolves params via `resolve_param` (`test_runnner.py:187`, returns *first* element value, not the list) and checks `keyword_map` membership without calling methods. The first-vs-list resolution divergence is a known dry-run vs execute pitfall. `DryRunExecutor.execute` returns `runner.result_printer.test_state` (`Dict[str, TestCaseResult]`, populated even under `NullResultPrinter` when `use_printer=False`) and `ExecutionEngine.execute` propagates it — the CLI ignores the return, the REST dry-run endpoints surface it as `DryRunResponse`. Dry-run is a **validator**: an unknown keyword or an unresolved `${var}` is recorded as a per-keyword `FAIL` (with `reason`) inside the result, not raised — `_dry_run_module` (`test_runnner.py:686`) catches `ValueError`/`OpticsError`, and `_init_keywords` resolves the display name defensively (`_safe_resolve`) so an undefined variable can't crash runner construction.
...
3. **HTTP server (`optics serve`)** — `helper/serve.py` mounts the FastAPI app from `common/expose_api.py`. `discover_keywords()` (`expose_api.py:366`) reflects over `optics_framework.api.*`; `_extract_keywords_from_class` (`:339`) collects public methods (skipping `_*` and `test*`) and humanises names via `_humanize_keyword` (`:203`). Adding to the API class suffices — but parameter types and docstrings flow into the OpenAPI schema, so keep them accurate. Beyond per-keyword `/v1/sessions/{id}/action`, two endpoints dry-run a **whole suite** without a device: `POST /v1/dry_run` (inline JSON suite, body read with a streaming size cap via `_read_body_capped`) and `POST /v1/dry_run/upload` (multipart CSV/YAML files or a single `.zip`). Both build a `LoadedSuite` (`build_suite_from_inline` / `load_suite_from_folder` in `helper/execute.py`; the folder loader calls `find_files(..., validate=False)` so a missing-test-cases upload yields a clean `400` instead of the CLI's `sys.exit`) and run it through `_execute_dry_run`, which creates an **ephemeral device-less session** (`SessionManager.create_session(..., require_driver=False)`), strips all source configs via `_strip_sources_for_dry_run` so no driver/element engine is instantiated, runs `mode="dry_run"` with `use_printer=False`, returns a `DryRunResponse`, and terminates the session in `finally`. Upload safety (zip-slip via `commonpath`, zip-bomb via a chunked written-byte counter that does NOT trust header sizes, entry/size caps, filename sanitisation) lives in `common/dry_run.py`; blocking suite-building/extraction is offloaded with `asyncio.to_thread`. These endpoints are **unauthenticated** like the rest of the server.
