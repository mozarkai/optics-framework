...
async def execute_keyword(session_id: str, request: ExecuteRequest):
    """
    Executes a single keyword directly and returns the result (synchronously),
    while still publishing events to the session's event queue.
    """
...
async def get_elements(session_id: str):
    requests = ExecuteRequest(
        mode="keyword",
        keyword="get_interactive_elements",
        params=[]
    )
    return await execute_keyword(session_id, requests)

@app.get("/session/{session_id}/source")
async def get_pagesource(session_id: str):
    requests = ExecuteRequest(
        mode="keyword",
        keyword="capture_pagesource",
        params=[]
    )
    return await execute_keyword(session_id, requests)
...
async def screen_elements(session_id: str):
    request = ExecuteRequest(
        mode="keyword",
        keyword="capture_and_get_screen_elements",
        params=[]
    )
    return await execute_keyword(session_id, request)
