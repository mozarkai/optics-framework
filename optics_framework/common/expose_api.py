...
async def _terminate_under_keyword_lock(session: Session, session_id: str) -> None:
    ...
    the worse failure. ``terminate_session`` never re-acquires the lock, so
    there is no nesting risk.
    """
    lock_acquired = False
    try:
        ...
        internal_logger.warning(
            ...
            session_id, SESSION_TEARDOWN_LOCK_TIMEOUT_S,
        )
    try:
...
async def delete_session(session_id: str):
    ...
    try:
        await _terminate_under_keyword_lock(session, session_id)
