    ...
    async def execute(self, session: Session, runner: Runner) -> None:
        ...
        if method:
            ...
            if heal_info:
                ...
                # sync by convention rather than a shared import. "suggested_steps" is the
                # clean, replayable recovery sequence (list of {keyword, params}); the
                # event extra only carries the string summary since extra values are strings.
                return {
                    "result": result,
                    "healed": True,
