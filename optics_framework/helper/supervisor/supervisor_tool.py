...
from optics_framework.helper.supervisor.routing_store import (
...
def run_supervisor(supervisor_config: SupervisorConfig) -> None:
    """Run the supervisor server (blocking). Entry point for `optics supervise`
    and the standalone script path."""
    global config
    config = supervisor_config

    if WORKER_MODE == "pool":
        logger.info(
            f"Starting supervisor with {config.num_workers} workers on ports "
            f"{config.base_port}-{config.base_port + config.num_workers - 1}"
        )
    else:
        logger.info("Starting supervisor with one worker per session (launcher-managed)")
...
def main() -> None:
    """Standalone CLI (kept for `python -m ...supervisor_tool`); the
    first-class entry point is `optics supervise`."""
    import argparse
