...
### `except OSError` around `rmtree` is too narrow

`optics_framework/common/session_manager.py` — the temp-directory cleanup catches `OSError` only. A non-`OSError` would skip `cleanup_junit` and `remove_session` *and* swallow the captured driver error, defeating the unconditional-cleanup guarantee added in Phase 0.

**Cost today:** low — `rmtree` overwhelmingly raises `OSError` subclasses.

**Fix:** broaden to `except Exception`. Two characters.

### A cancelled `DELETE` during lock acquisition skips teardown

`optics_framework/common/expose_api.py` — the `except asyncio.TimeoutError` around the bounded `keyword_lock` acquisition does not catch `CancelledError`, so a cancellation while waiting propagates before `terminate_session` is submitted.

**Cost today:** transient; a retried `DELETE` succeeds. Starlette does not cancel plain HTTP handlers on client disconnect, so this is unlikely to fire.

**Fix:** catch `BaseException` around the acquisition and fall through to teardown, as the timeout path already does.
